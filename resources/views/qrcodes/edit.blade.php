@extends('layouts.app')
@section('title', 'Modifier un QR Code')

@section('content')
<div class="mb-8 space-y-0.5">
    <h1 class="text-xl font-semibold tracking-tight">Modifier un QR Code</h1>
</div>

<form method="POST" action="{{ route('qrcodes.update', $qrcode) }}" enctype="multipart/form-data" class="space-y-4">
    @csrf
    @method('PUT')
    <div>
        <x-form-label for="title" value="Titre" />
        <x-form-input id="title" placeholder="Titre" name="title" type="text" :value="$qrcode->title" required />
    </div>
